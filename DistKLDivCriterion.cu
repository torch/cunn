...
#define DISTKLDIVCRITERION_THREADS 128

__global__ void cunn_DistKLDivCriterion_updateOutput_kernel(float* output, float *input, float *target, int nframe, int dim, int sizeAverage)
{
  __shared__ float buffer[DISTKLDIVCRITERION_THREADS];
  int k = blockIdx.x;
  ...
  {
    buffer[threadIdx.x] += target_k[i] > 0 ? target_k[i] * (log(target_k[i]) - input_k[i]) : 0;
  }
  __syncthreads();
...
__global__ void cunn_DistKLDivCriterion_updateGradInput_kernel(float *gradInput, float *input, float *target, float norm, int nframe, int dim)
{
  int k = blockIdx.x;
  ...
  // gradInput
  for (int i=i_start; i<i_end; i+=i_step)
    gradInput_k[i] = target_k[i] > 0 ? norm*(- target_k[i]) : 0;
}

static int cunn_DistKLDivCriterion_updateOutput2(lua_State *L)
{
  THCudaTensor *input = (THCudaTensor*)luaT_checkudata(L, 2, "torch.CudaTensor");
  THCudaTensor *target = (THCudaTensor*)luaT_checkudata(L, 3, "torch.CudaTensor");
  int sizeAverage = luaT_getfieldcheckboolean(L, 1, "sizeAverage");
  ...
  dim3 blocks(1);
  dim3 threads(DISTKLDIVCRITERION_THREADS);

  cunn_DistKLDivCriterion_updateOutput_kernel<<<blocks,threads>>>(output->data,
...
static int cunn_DistKLDivCriterion_updateGradInput2(lua_State *L)
{
  THCudaTensor *input = (THCudaTensor*)luaT_checkudata(L, 2, "torch.CudaTensor");
  THCudaTensor *target = (THCudaTensor*)luaT_checkudata(L, 3, "torch.CudaTensor");
  int sizeAverage = luaT_getfieldcheckboolean(L, 1, "sizeAverage");
  ...
  cunn_DistKLDivCriterion_updateGradInput_kernel<<<blocks,threads>>>(THCudaTensor_data(gradInput),
							THCudaTensor_data(input),
							THCudaTensor_data(target),
							norm,
							1, size);
...
static const struct luaL_Reg cunn_DistKLDivCriterion__ [] = {
  ...
  {"DistKLDivCriterion_updateOutput2", cunn_DistKLDivCriterion_updateOutput2},
  {"DistKLDivCriterion_updateGradInput2", cunn_DistKLDivCriterion_updateGradInput2},
  {NULL, NULL}
};
