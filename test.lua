...
local function constructComplexConvolution(from, to, ki, kj, outi, outj, w, b)
   ...
   local convs = {}  -- For calculating grad weights we need to save conv handle
...
function cunntest.SpatialConvolutionComplexMM_checkGT()
  -- We have a ground truth CPU based convolution, but we also need to check
  -- it for correctness.  One simple way of doing this is just to match known
  -- values against those created in matlab.  I know this is messy, but it's
  -- the quickest first order check I can think of.  Also, this is only true
  -- 2D correlation, not a 2D correlation on 3D tensors (ie, to=from=1).  Still
  -- this test is better than nothing.

  -- Construct GT input and output values generated by Matlab.
  -- MATLAB: rng(0);
  -- MATLAB: z = complex(rand(4), rand(4));
  local zreal = torch.Tensor({{0.8147237, 0.6323593, 0.9575068, 0.9571670},
                              {0.9057919, 0.0975404, 0.9648885, 0.4853756},
                              {0.1269868, 0.2784982, 0.1576131, 0.8002805},
                              {0.9133759, 0.5468815, 0.9705928, 0.1418863}})
  local zimag = torch.Tensor({{0.4217613, 0.6557407, 0.6787351, 0.6554779},
                              {0.9157355, 0.0357117, 0.7577401, 0.1711867},
                              {0.7922073, 0.8491293, 0.7431325, 0.7060461},
                              {0.9594924, 0.9339932, 0.3922270, 0.0318328}})
  local z = torch.cat(zreal:resize(4,4,1), zimag:resize(4,4,1), 3)
  z = z:resize(1,4,4,2)  -- (nfilts, H, W, 2)
  -- MATLAB: filt = complex(rand(2), rand(2))
  local filtreal = torch.Tensor({{0.2769230, 0.0971318},
                                 {0.0461714, 0.8234578}})
  local filtimag = torch.Tensor({{0.6948286, 0.9502221},
                                 {0.3170995, 0.0344461}})
  local filt = torch.cat(filtreal:resize(2,2,1), filtimag:resize(2,2,1), 3)
  filt = filt:resize(1,1,2,2,2)
  -- MATLAB: bias = complex(rand(1), rand(1))
  local bias = torch.Tensor({0.4387444, 0.3815584})
  bias = bias:resize(1,2)
  -- MATLAB: out = filter2(filt, z, 'valid') + bias  // 2D correlation
  local outreal = torch.Tensor({{-0.3598362,  0.3679102, -0.0995218},
                                {-0.0164230, -0.3375682,  0.4702317},
                                {-0.7002700, -0.2501511, -0.5909471}})
  local outimag = torch.Tensor({{ 2.0912962,  2.6680830,  2.7066438},
                                { 2.1463308,  2.1945653,  2.4329247},
                                { 2.1581597,  1.6051148,  1.8828713}})
  local out = torch.cat(outreal:resize(3,3,1), outimag:resize(3,3,1), 3)
  out = out:resize(1,3,3,2)
  ...
  cconv = constructComplexConvolution(from,to,ki,kj,outi,outj, 
    filt:resize(1,1,kj,ki,2), bias:resize(1,2))
  local out_gt = cconv:forward(z:resize(1,to,inj,ini,2))[1]
  local precision = 1e-5

  local gconv = nn.SpatialConvolutionComplexMM(from,to,ki,kj,si,sj):cuda()
  gconv.weight:copy(filt)
  gconv.bias:copy(bias)
  local out_gpu = gconv:forward(z[1]:cuda()):float()

  mytester:assertlt((out_gt - out):abs():max(), precision, 
    'ground-truth complex convolution is incorrect')
  mytester:assertlt((out_gpu - out):abs():max(), precision, 
    'gpu complex convolution is incorrect')
end
 
function cunntest.SpatialConvolutionComplexMM_forward_single()
   local from = math.random(1,32)
   local to = math.random(1,8) * 8
   local ki = math.random(3,15)
   local kj = math.random(3,15)
   ...
   local input = torch.randn(1,from,inj,ini,2)  -- GT network works with batches
   local gconv = nn.SpatialConvolutionComplexMM(from,to,ki,kj,si,sj)

   local bias = gconv.bias:clone():resize(to, 2)
   local weight = gconv.weight:clone():resize(to, from, kj, ki, 2)
   local sconv = constructComplexConvolution(from, to, ki, kj, outi, outj,
    weight, bias)

   local groundtruth = sconv:forward(input)[1]

   gconv = gconv:cuda()
   input = input:resize(from,inj,ini,2):cuda()
   local rescuda = gconv:forward(input)
...
function cunntest.SpatialConvolutionComplexMM_forward_batch()
   local bs = math.random(1,4) * 4
   local from = math.random(1,32)
   local to = math.random(1,8) * 8
   local ki = math.random(3,15)
   ...
   local input = torch.randn(bs,from,inj,ini,2)
   local gconv = nn.SpatialConvolutionComplexMM(from,to,ki,kj,si,sj)  
...
function cunntest.SpatialConvolutionComplexMM_backward_single()
   local from = math.random(1,32)
   local to = math.random(1,8) * 8
   local ki = math.random(3,15)
   local kj = math.random(3,15)
   ...
   local input = torch.randn(1,from,inj,ini,2)
   local gradOutput = torch.randn(1,to,outj,outi,2)
   local gconv = nn.SpatialConvolutionComplexMM(from,to,ki,kj,si,sj)

   local bias = gconv.bias:clone():resize(to, 2)
   local weight = gconv.weight:clone():resize(to, from, kj, ki, 2)
   local sconv, convs, grad_func = constructComplexConvolution(from, to, ki, kj,
     outi, outj, weight, bias)

   local parameters, gradParameters = sconv:getParameters()
   gradParameters:zero()

   sconv:forward(input)
   local groundgrad = sconv:backward(input, gradOutput)[1]
   local groundweight, groundbias = grad_func(convs)  

   input = input:resize(from,inj,ini,2):cuda()
   gconv = gconv:cuda()
   gradOutput = gradOutput:resize(to,outj,outi,2):cuda()
   gconv:forward(input)
   gconv:zeroGradParameters()
   local rescuda = gconv:backward(input, gradOutput)
   local weightcuda = gconv.gradWeight
...
function cunntest.SpatialConvolutionComplexMM_backward_batch()
   local bs = math.random(1,4) * 4
   local from = math.random(1,32)
   local to = math.random(1,8) * 8
   local ki = math.random(3,15)
   ...
   local input = torch.randn(bs,from,inj,ini,2)
   local gradOutput = torch.randn(bs,to,outj,outi,2)
   local gconv = nn.SpatialConvolutionComplexMM(from,to,ki,kj,si,sj)
